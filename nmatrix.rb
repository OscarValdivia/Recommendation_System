...
class Nmatrix
    ...
    def create_matrix(users, ads, type = 1) #create a specific type of matrix, then store it
                                            #users and ads: arrays with all the users and ads information (in the form given by parse.rb)
                                            #type: integer with the type of matrix requeted.
        matrix = {}
        ...
        case type
            ...
            when 7, 8 #7:Matrix Geolocalization-Ad | 8:Matrix Ad-Geolocalization
                ...
                geo(users, n, n) #generates the radius for each user, given longitud and latitude 
        ...
        if (type % 2) == 0
             matrix = invert(matrix) #if the type is even, then an "reverse" type of matrix was requested. Invert does that
        end 	

        for ix,iy in matrix #sorting each user on the matrix in descending order
    ...
    def get_data_user(id, num = @matrix[id].length) #WARNING: call this method with existing ids...
                                                    #returns and array with the item clicked by the user "id"
        if num <= 0
            return nil
        end
    ...
    def invert(matrix = @matrix, override = false) #override tells the class to override the original matrix with the inverted one
                                                   #"inverts" the current matrix and return it
    ...
    def normalize #normalize the values on the matrix
    ...
    def un_normalize #un_normalize the matrix
    ...
    def distance(u1, u2, function = method(:metric_manhattan)) #WARNING: call this method with existing user ids
                                                               #u1, u2: arrays with the info of each user (clicked items)
                                                               #function: the metric function to use.
                                                               #returns the distance between u1 and u2 using the given function
        check1 = false
        check2 = false       
